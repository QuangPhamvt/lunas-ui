import { Lucide2CircleAlert, Lucide2UserIcon } from '@/Icons'
import { cn } from '@/libs'
import {
  ChangeEvent,
  forwardRef,
...
interface IFullnameInputProps extends HTMLAttributes<HTMLDivElement> {
  errorMessage?: string
  onClick?: () => void
  onBlur?: () => void
    ...
    const iconRender = useMemo(() => {
      return (
        <div className="flex items-center justify-center border-r border-solid border-r-ui-text-300 px-6 py-3">
          <Lucide2UserIcon size={16} color={isFocused ? '#606189' : '#B9BAC0'} />
        </div>
      )
    }, [isFocused])
